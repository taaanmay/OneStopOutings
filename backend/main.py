...
# The old parse_llm_response function is no longer needed and has been removed.
...
async def generate_plan_with_llm(preferences: UserPreferences):
    ...
    # --- NEW: Prompt engineering to request a JSON response ---
    prompt = f"""
...
async def get_single_replacement_event(request: RegenerateRequest):
    ...
    # --- NEW: Prompt engineering for a single JSON object ---
    prompt = f"""
...
async def create_outing_plan(preferences: UserPreferences):
    ...
    final_plan = OutingPlan(plan=parsed_events, total_cost=total_cost, total_duration=total_duration)
    
    api_cache[cache_key] = (final_plan, time.time())
    logging.info(f"Successfully created and cached plan with {len(parsed_events)} events.")
    return final_plan
